...
const getTable = async (req, res) => {
    try {
        ...
        Connection.query('SELECT SUM(units) AS total_units FROM donation WHERE bloodtype = ?',[user_bloodtype], (error, rows) => {
            if (error) {
        ...
        Connection.query('SELECT * FROM bdtable WHERE bloodgroup = ?', [user_bloodtype], (error, rows) => {
            if (error) {
            ...
            if (rows.length===0) {
                const newDonation = { bloodgroup: user_bloodtype, units: totalOftheKind };
                Connection.query("INSERT INTO bdtable SET ?", newDonation, (err, result) => {
                    if (err) {
                ...
                Connection.query("UPDATE bdtable SET ? WHERE bloodgroup = ?", [updatedCount ,user_bloodtype], (err, rows) => {
                    if (err) {
